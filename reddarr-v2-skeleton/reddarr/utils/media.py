...
def make_thumb(
    ...
    try:
        ...
        is_video = any(source_lower.endswith(ext) for ext in (".mp4", ".webm", ".mov", ".avi", ".mkv"))
...
def detect_image_corruption(path: str) -> bool:
    ...
    Uses Pillow to attempt opening and verifying the image.
    Returns True if the image appears corrupted.
    """
    if not path or not os.path.exists(path):
        return True

    # Skip non-image files
    if not any(path.lower().endswith(ext) for ext in (".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp")):
        return False
